...
class watermark():
    ...
    def extract_watermark(self,file, need_img):                           #传入一张图片
        if not self.wm_shape:
            print("水印的形状未设定")
            return 0

        # 读取图片
        # embed_img = cv_imread(file).astype(np.float32)
        embed_img = file
        if self.color_mod == 'RGB':
            embed_img_YUV = embed_img
        elif self.color_mod == 'YUV':
        ...
        extract_wm_U[wm_index] = extract_wm_U.copy()
        extract_wm_V[wm_index] = extract_wm_V.copy()
        extract_wm_output = extract_wm.reshape(self.wm_shape[0], self.wm_shape[1])
        extract_wm_output = extract_wm.reshape(self.wm_shape[0], self.wm_shape[1])
        ret, wm_out = cv2.threshold(extract_wm_output, 127, 255, cv2.THRESH_BINARY)
        
        if need_img == 1:
            # return extract_wm.reshape(self.wm_shape[0], self.wm_shape[1])       #返回一个图片
            return wm_out  # 返回一个图片
        else:
            return image_transform(wm_out)                           #返回一个Tensor


        # path, file_name = os.path.split(out_wm_name)
        # if not os.path.isdir(os.path.join(path, 'Y_U_V')):
        #     os.mkdir(os.path.join(path, 'Y_U_V'))
        # cv_imwrite(os.path.join(path, 'Y_U_V', 'Y' + file_name),
        #            extract_wm_Y.reshape(self.wm_shape[0], self.wm_shape[1]))
        # cv_imwrite(os.path.join(path, 'Y_U_V', 'U' + file_name),
        #            extract_wm_U.reshape(self.wm_shape[0], self.wm_shape[1]))
        # cv_imwrite(os.path.join(path, 'Y_U_V', 'V' + file_name),
...
if __name__=="__main__":
    k = 5000
    image_path = r'E:\NN\trans\download\image\image_data\image_data\\'
    watermark_path = r'E:\NN\trans\download\bi-dataset\\'

    for i in range(4001,4050):
        bwm1 = watermark(4399,2333,36,20,wm_shape=(32,32))
        bwm1.read_ori_img(image_path + str(i) + ".jpg")
        bwm1.read_wm(watermark_path + "watermark" + str(i) + ".png")
        bwm1.embed(r'E:\NN\show\Iw\\'+'image_embedded' + str(i) + '.png')
        bwm1.extract(r'E:\NN\show\Iw\\'+'image_embedded' + str(i) + '.png',r'E:\NN\show\Wa\\' + str(i) + ".png")
