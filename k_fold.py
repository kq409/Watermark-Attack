...
def k_fold(k, learning_rate=learning_rate):
    ...
    for i in range(k):
        ...
        for num in range(epoch):
            ...
            if num == 0:
                ...
                torch.save(checkpoint, './checkpoints/UD_kfold_{}fold.pth'.format(i))
                print('Model Saved')
            if loss_min > total_validation_loss:
                loss_min = total_validation_loss
                torch.save(checkpoint, './checkpoints/UD_kfold_{}fold.pth'.format(i))
                print('Model Saved')
            print('Loss2_min = {}'.format(loss_min))
            print("第{}epoch测试集上的Loss:{}".format(i+1, total_validation_loss))
    writer.close()


k_fold(4, learning_rate = learning_rate)